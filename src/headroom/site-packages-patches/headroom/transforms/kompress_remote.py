...
_REQUEST_CONTEXT: ContextVar[tuple[str | None, str | None]] = ContextVar(
    "headroom_kompress_request_context",
    default=(None, None),
)


def set_compression_request_context(
    request_id: str | None,
    correlation_id: str | None,
) -> Token[tuple[str | None, str | None]]:
    """Bind opaque request identifiers to the current compression worker.

    The ContentRouter API predates per-request broker metadata and does not
    accept arbitrary keyword arguments.  A context variable keeps the
    identifiers out of prompt text while allowing executor-thread compression
    to attach the same receipt to the broker call.
    """

    return _REQUEST_CONTEXT.set((request_id, correlation_id))


def reset_compression_request_context(
    token: Token[tuple[str | None, str | None]],
) -> None:
    _REQUEST_CONTEXT.reset(token)


def _compression_request_context() -> tuple[str | None, str | None]:
...
class RemoteKompressCompressor:
    ...
    def compress(
        ...
        *,
        ...
        request_id, correlation_id = _compression_request_context()
        request_id = _safe_request_id(request_id)
        correlation_id = _safe_request_id(correlation_id) or request_id
        request_headers = dict(self._headers)
        if request_id is not None:
            request_headers["x-headroom-request-id"] = request_id
        if correlation_id is not None:
            request_headers["x-headroom-correlation-id"] = correlation_id
        request_payload = {
            "content": content,
