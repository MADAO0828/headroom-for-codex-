    ...
    def eager_load_compressors(self) -> dict[str, str]:
        ...
        if self.config.enable_kompress:
            ...
            if compressor:
                ...
                elif getattr(compressor, "is_remote", False):
                    # Remote Kompress has no local model to load.  Probe the
                    # broker's bounded /readyz contract instead of marking it
                    # deferred (which would make /health lie about readiness).
                    remote_status = compressor.preload(allow_download=False)
                    status["kompress"] = str(remote_status)
                    provider = getattr(compressor, "provider", None)
                    backend = getattr(compressor, "backend", None)
                    preload_error = getattr(compressor, "preload_error", None)
                    if isinstance(provider, str) and provider:
                        status["kompress_provider"] = provider
                    if isinstance(backend, str) and backend:
                        status["kompress_backend"] = backend
                    if isinstance(preload_error, str) and preload_error:
                        status["kompress_error"] = preload_error
                else:
