...
def onnx_thread_spinning_enabled() -> bool:
    """Whether ONNX Runtime thread pools may spin-wait while idle.

    A long-lived proxy keeps its models loaded between requests. ORT's default
    spin-wait can then keep every worker core busy while the proxy is idle,
    competing with the gateway and helper processes. Blocking idle workers is
    the safer proxy default; set ``HEADROOM_ONNX_ALLOW_SPINNING=1`` to restore
    the throughput-oriented ORT default for a dedicated batch host.
    """
...
def create_cpu_session_options(
    ...
    *,
    ...
    if not onnx_thread_spinning_enabled():
        # Best-effort for compatibility with older ORT builds that do not
        # recognize these session configuration keys.
        for spin_key in (
