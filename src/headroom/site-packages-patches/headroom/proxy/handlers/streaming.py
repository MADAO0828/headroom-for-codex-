...
class StreamingMixin:
    ...
    def _parse_sse_usage_from_buffer(
        ...
        located. Invalid UTF-8 is replacement-decoded for diagnostics;
        the original bytes remain untouched on the client stream.
        """
        from headroom.proxy.helpers import parse_sse_events_from_byte_buffer
        ...
        for _event_name, data_str in events:
            ...
            # A valid SSE frame can still carry a JSON scalar or array.  Usage
            # extraction is best-effort and must never turn that upstream
            # payload into a stream-level exception during finalization.
            if not isinstance(data, dict):
                continue
    ...
    def _parse_sse_usage_from_buffer_safe(
        self, stream_state: dict[str, Any], provider: str
    ) -> dict[str, int] | None:
        """Best-effort usage parsing that cannot terminate a forwarded stream."""
        try:
            return self._parse_sse_usage_from_buffer(stream_state, provider)
        except Exception as parse_error:
            logger.warning(
                "Ignoring SSE usage parse failure for %s (%s)",
                provider,
    ...
    async def _finalize_stream_response(
        ...
        *,
        ...
        if isinstance(sse_buffer, bytearray) and len(sse_buffer) > 0:
            sse_buffer.extend(b"\n\n")
            try:
                late_usage = self._parse_sse_usage_from_buffer_safe(stream_state, provider) or {}
            except Exception as parse_error:
                # Residual usage is optional telemetry.  A malformed tail
                # must not abort response cleanup or propagate out of an ASGI
                # streaming generator.
                logger.warning(
                    "[%s] Ignoring residual SSE usage parse failure (%s)",
                    request_id,
                    type(parse_error).__name__,
                )
                late_usage = {}
        ...
        # Keep the streaming ledger internally consistent even when an
        # upstream SSE omits usage. Responses compression can report savings
        # for structured input that the generic message counter cannot see;
        # without this invariant the fallback row becomes original=3,
        # optimized=0, saved=1027 and produces an impossible savings rate.
        effective_tokens_saved = max(int(tokens_saved), 0)
        effective_optimized_tokens = max(int(optimized_tokens), 0)
        effective_original_tokens = max(
            int(original_tokens), effective_optimized_tokens + effective_tokens_saved
        )
        if (
            ...
            effective_original_tokens = max(
                ...
                provider_input_tokens + effective_tokens_saved,
            )
            ...
            try:
                async with contextlib.aclosing(upstream_response) as response:
                    ...
                    async for chunk in response.aiter_bytes():
                        ...
                        usage = self._parse_sse_usage_from_buffer_safe(stream_state, provider)
                        if usage:
                ...
                # complete events). Diagnostic reconstruction must not turn
                # an upstream byte defect into a second stream failure.
                full_sse_data: str = (
                    full_sse_bytes.decode("utf-8", errors="replace") if full_sse_bytes else ""
                )
                ...
                try:
                    ...
                    # Finalization is telemetry/accounting work after bytes
                    # have been sent.  It must not convert a usable stream
                    # into a client-side disconnect.
                    logger.warning(
                        "[%s] Stream finalization failed after forwarding; "
                        "preserving client stream cleanup (%s)",
                        request_id,
        ...
        async def generate():
            ...
            try:
                async for sse_chunk in self.anthropic_backend.stream_openai_message(body, headers):
                    ...
                    full_sse_bytes.extend(chunk_bytes)
                    _absorb(self._parse_sse_usage_from_buffer_safe(stream_state, "openai"))
