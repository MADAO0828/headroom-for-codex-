...
class HeadroomProxy(
    ...
    async def startup(self):
        ...
        # Remote Kompress reports a real broker readiness result from its
        # /readyz probe.  Promote that result to the warmup slot only when the
        # provider/backend metadata is present and bounded; a missing or
        # malformed contract must stay non-ready even if a generic transform
        # status happened to say "loaded".
        if eager_status.get("kompress") == "loaded":
            provider_raw = eager_status.get("kompress_provider", "")
            backend_raw = eager_status.get("kompress_backend", "")
            ...
            if provider and backend:
                ...
                for pipeline in (self.anthropic_pipeline, self.openai_pipeline):
                    for transform in pipeline.transforms:
                        candidate = getattr(transform, "_kompress_remote", None)
                        if getattr(candidate, "is_remote", False):
                            remote_handle = candidate
                            break
                    if remote_handle is not None:
                        break
                if remote_handle is not None:
                    self.warmup.kompress.mark_loaded(
                    ...
                    self.warmup.kompress.mark_error("kompress_readiness_handle_missing")
            else:
                self.warmup.kompress.mark_error("kompress_readiness_metadata_missing")
        elif eager_status.get("kompress_error"):
            # Keep the failure code deliberately small and body-free.  The
            # independent broker monitor remains responsible for live recovery.
            self.warmup.kompress.mark_error(str(eager_status["kompress_error"]))

        # Update internal status from eager loading results
        if (
            eager_status.get("kompress") in {"enabled", "loaded"}
            and self.warmup.kompress.status == "loaded"
        ):
...
def create_app(config: ProxyConfig | None = None) -> FastAPI:
    ...
    def _kompress_readiness() -> tuple[bool, str | None]:
        """Return the warmup snapshot plus a live remote-broker verdict.

        Local Kompress remains a startup snapshot.  A remote compressor keeps
        a small handle in the slot so a failed request can immediately make
        health/debug non-ready without probing the broker on every health poll.
        The independent monitor remains responsible for live recovery.
        """

        slot = proxy.warmup.kompress
        ready = slot.status == "loaded"
        error = slot.error
        handle = slot.handle
        if ready and getattr(handle, "is_remote", False):
            try:
                ready = bool(handle.is_ready())
            except Exception:
                ready = False
            if not ready:
                error = getattr(handle, "preload_error", None) or "broker_not_ready"
        return ready, error
    ...
    def _health_checks() -> dict[str, dict[str, Any]]:
        ...
        kompress_ready, _kompress_error = _kompress_readiness()
        return {
            ...
            "kompress": _component_health(
                ...
                ready=kompress_ready,
                provider=proxy.warmup.kompress.info.get("provider", None),
                backend=proxy.warmup.kompress.info.get("backend", None),
                error=_kompress_error,
            ),
        }
    ...
    async def debug_warmup():
        ...
        if warmup_registry is not None and isinstance(payload.get("kompress"), dict):
            kompress_ready, kompress_error = _kompress_readiness()
            kompress_payload = payload["kompress"]
            if not kompress_ready and kompress_payload.get("status") == "loaded":
                # Do not mutate the registry from a read-only endpoint; return
                # an honest live view when a previously loaded remote broker
                # has since failed.
                kompress_payload["status"] = "error"
                kompress_payload["error"] = kompress_error or "broker_not_ready"
                kompress_payload.setdefault("info", {})["ready"] = False
